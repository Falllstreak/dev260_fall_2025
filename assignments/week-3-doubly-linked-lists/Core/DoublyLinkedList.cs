...
namespace Week4DoublyLinkedLists.Core
{
    ...
    public class DoublyLinkedList<T> : IEnumerable<T>
    {
        ...
        public void AddFirst(T item)
        {
            ...
            // 1. Create a new node with the item                                       New Node created
            // 2. If list is empty: set both head and tail to new node                  Condition for if list is empty
            // 3. If list is not empty:                                                 
            //    - Set new node's Next to current head                                 
            //    - Set current head's Previous to new node
            //    - Update head to new node                                             Implemented conditions for list if NOT empty
            // 4. Increment count                                                       counts
        ...
        public void AddLast(T item)
        {
            ...
            // 4. Increment count                                                       Followed steps to implement adding to the end of list
            ...
            // Creates new node with the item
            var node = new Node<T>(item);
        ...
        public void Insert(int index, T item)
        {
            ...
            if (index < 0 || index > count)
                throw new ArgumentOutOfRangeException(nameof(index), "Index has to be between 0 and Count, inclusive.");
            ...
            // Traverses to the current node at the given index
            var current = head;
            for (int i = 0; i < index; i++)
                current = current!.Next;

            // Now the current is the node that will come after the new node
            var newNode = new Node<T>(item);

            // Connects the new node with its neighbors
            newNode.Previous = current!.Previous;
            newNode.Next = current;

            // Adjusts the existing nodes to point to the new one
        ...
        public void DisplayForward()
        {
            ...
            // empty list message if empty
            if (IsEmpty)
            {
                Console.WriteLine("Forward: [Empty List]");
                return;
            }
            
            // starts from head node
            var current = head;
            Console.Write("Forward: Head -> ");

            // traverses using Next pointers until null
            while (current != null)
            {
        ...
        public void DisplayBackward()
        {
            ...
            // empty list message if list is empty
            if (IsEmpty)
            {
                Console.WriteLine("Backward: [Empty List]");
                return;
            }

            // starts from tail node
            var current = tail;

            // traverses using previous pointers until null
            Console.Write("Backward: Tail -> ");
            while (current != null)
            {
                Console.Write($"[{current.Data}]");
        ...
        public T[] ToArray()
        {
            ...
            // creates array of size count
            T[] array = new T[count];

            // traverses the list and copies elements to array
            var current = head;
            int i = 0;
            while (current != null)
            {
        ...
        public bool Contains(T item)
        {
            ...
            // compares each node's data with the item
            while (current != null)
            {
        ...
        public int IndexOf(T item)
        {
            ...
            // traverses the list until we reach the end
            while (current != null)
            {
                ...
                // increments index as it moves forward
                index++;
            }
        ...
        public T RemoveFirst()
        {
            ...
            // stores the data of current head to return later
            T removedData = head!.Data;

            // updates head to point to the next node
            head = head.Next;

            // if the new head exists it sets its Previous to null
            if (head != null)
                head.Previous = null;
            else
            // if the list became empty it sets tail to null as well
                tail = null;
        ...
        public T RemoveLast()
        {
            ...
            // stores data of the current tail to return later
            T removedData = tail!.Data;

            // updates tail to point to previous node
            tail = tail.Previous;

            // if the new tail exists it sets its Next to null
            if (tail != null)
                tail.Next = null;
            else
            // if the list became empty it sets head to null as well
                head = null;
        ...
        public bool Remove(T item)
        {
            ...
            // returns true since removal succeeded
            return true;
        }
        ...
        public T RemoveAt(int index)
        {
            ...
            // middle removal: get the node at the specified index
            var current = GetNodeAt(index); // uses GetNodeAt helper
            T removedData = current.Data;

            // RemoveNode helper remove the node safely
            RemoveNode(current);
        ...
        public void Reverse()
        {
            ...
            // checks if list is empty or has only one element
            if (IsEmpty || count == 1)
            // nothing to reverse
                return;
            ...
            // traverses list and swaps Next and Previous for each node
            while (current != null)
            {
                // swap pointers
                ...
                // moves to the next node in original order (this is Previous after the swap)
                current = current.Previous;
            }
        ...
        public Node<T> GetNodeAt(int index)        // changed this to public because the MusicPlaylist needs it
        {
            ...
            // validates index range
            if (index < 0 || index >= count)
                throw new ArgumentOutOfRangeException(nameof(index), "Index has to be between 0 and Count-1.");

            Node<T>? current;

            // optimizes traversal direction
            if (index < count / 2)
            {
            // starts from head and goes forward
                current = head;
                for (int i = 0; i < index; i++)
                    current = current!.Next;
            }
            ...
            {
            // starts from tail and goes backward
                current = tail;
        ...
        private void RemoveNode(Node<T> node)
        {
            ...
            if (node == null)
                throw new ArgumentNullException(nameof(node), "Node to remove cannot be null.");

            // only node in list (head == tail == node)
            if (node == head && node == tail)
            {
                head = null;
                tail = null;
            }
            // first node (node == head, but not tail)
            else if (node == head)
            {
                head = node.Next;
                head!.Previous = null;
            }
            // last node (node == tail, but not head)
            else if (node == tail)
            {
                tail = node.Previous;
                tail!.Next = null;
            }
            // middle node (node has both Previous and Next)
            else
            {
            ...
            // decrements count for all cases
            count--;
        }
