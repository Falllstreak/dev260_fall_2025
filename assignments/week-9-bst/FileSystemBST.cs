...
namespace FileSystemNavigator
{
    ...
    public class FileSystemBST
    {
        ...
        public bool CreateFile(string fileName, long size = 1024)
        {
            ...
            {
                // This occurs if the file already exists
                return false;
            }
        }
        ...
        if (string.IsNullOrWhiteSpace(directoryName))
            ...
            try
            {
                root = InsertNode(root, newDir); // Use the same InsertNode logic
                return true;
            }
            catch (InvalidOperationException)
            {
        ...
        public FileNode? FindFile(string fileName)
        {
            ...
            // Use the recursive helper that properly navigates the BST
        ...
        public List<FileNode> FindFilesByExtension(string extension)
        {
            ...
            // Use TraverseAndCollect with a filter
            TraverseAndCollect(root, results, f => f.Type == FileType.File && f.Extension == extension);
        ...
        public List<FileNode> FindFilesBySize(long minSize, long maxSize)
        {
            ...
            // Use TraverseAndCollect with a filter for files within the size range
        ...
        public List<FileNode> FindLargestFiles(int count)
        {
            ...
            if (count <= 0)
                return new List<FileNode>(); // Returns an empty list for invalid count
            
            var allFiles = new List<FileNode>();

            // Collect all files using the helper
            TraverseAndCollect(root, allFiles, f => f.Type == FileType.File);

            // Sort descending by size
            var topFiles = allFiles.OrderByDescending(f => f.Size).Take(count).ToList();
        ...
        public long CalculateTotalSize()
        {
            ...
            // Use the existing helper to collect all files
            TraverseAndCollect(root, allFiles, f => f.Type == FileType.File);
        ...
        public bool DeleteItem(string fileName)
        {
            ...
            // Find nodes matching the file name using existing helper
            var matchingNodes = new List<FileNode>();
            TraverseAndCollect(root, matchingNodes, f => f.Name.Equals(fileName, StringComparison.OrdinalIgnoreCase));

            if (matchingNodes.Count == 0)
                return false; // nothing to delete

            FileNode nodeToDelete = matchingNodes[0];
            deleted = true;

            // Rebuild the tree without the node to delete
            var remainingNodes = new List<FileNode>();
            TraverseAndCollect(root, remainingNodes, f => f != nodeToDelete);

            root = null; // clear current tree
            foreach (var f in remainingNodes)
            {
                root = InsertNode(root, f); // re-insert remaining nodes
            }
        ...
        private TreeNode? InsertNode(TreeNode? node, FileNode fileData)
        {
            ...
            // Base case: empty spot in BST, create new node
            ...
            {
                // Duplicate file/directory name
                throw new InvalidOperationException("File or directory already exists.");
            }
        ...
        private FileNode? SearchNode(TreeNode? node, string fileName)
        {
            ...
            // Only compare by name if types match? Or use CompareFileNodes with a dummy FileNode of type File
            var targetNode = new FileNode(fileName, FileType.File);

            int comparison = CompareFileNodes(targetNode, node.FileData);

            if (comparison == 0) return node.FileData;
            if (comparison < 0) return SearchNode(node.Left, fileName);
